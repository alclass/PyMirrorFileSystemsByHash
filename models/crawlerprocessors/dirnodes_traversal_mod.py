...
class DirNode:
  ...
  def level(self):
    if self._level is None:
      self._level = self._get_n_levels()
    return self._level
  ...
  def _get_n_levels(self, level_counter=0):
    """
    ...
    if self.parentnode:
      level_counter += 1
      return self.parentnode._get_n_levels(level_counter)
    return level_counter

  def _get_root(self):
    if self.parentnode is None:
      return self
    return self.parentnode._get_root()
  ...
  def fill_in_to_dbentry(self, session, dbentry=None):
    ...
    if dbentry is None:
      dbentry = session.query(sam.FSEntryInDB).\
        ...
        filter(sam.FSEntryInDB.isfile == False).\
        first()
    if dbentry is None:
  ...
  def concatenate_sha1hex_of_children_files_via_db(self, session):
    dbentries = session.query(sam.FSEntryInDB).\
        ...
        filter(sam.FSEntryInDB.isfile == True).\
...
def traverse_topdown_leftright(dirnode, session=None):
  ...
  dirnodes = copy.copy(dirnode.children_dirnodes)
