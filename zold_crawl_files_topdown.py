...
class FileSweeper:
  ...
  def __init__(self, mount_abspath, store_sqlite_elsewhere=False):
    self.mount_abspath = mount_abspath
    self.middlepathobj = midpath.MiddlePath(self.mount_abspath)
    self.store_sqlite_elsewhere = store_sqlite_elsewhere
  ...
  def perfom_updir_sweep(self):
    ...
    if self.store_sqlite_elsewhere:
      self.session = con.get_session_for_sqlite_with_filepath(config.STORE_SQLITE_IN)
    else:
  ...
  def process_oswalk_abspath_iteration(self, abspath, files):
    ...
    for foldername in config.L1DIRS_TO_AVOID_IN_MIRRORING:
      if middlepath.startswith(foldername):
        return
      if 'z-del-error' in middlepath:
        return
