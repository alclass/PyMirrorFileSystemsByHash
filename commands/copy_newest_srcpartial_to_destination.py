#!/usr/bin/env python3
"""
commands/copy_newest_srcpartial_to_destination.py
  This script copy files from a partial dirtree to a destination
  The copied files are the ones that are missing in destination.
  Copies in this script must have the same relative positions,
    otherwise report it but do not perform copying
"""
import datetime
import os.path
import shutil
...
class SameRelativePathsToDestinationCopier:
  ...
  def mirror_by_copying_across_dirtrees(self):
    for i, src_rowlist in enumerate(self.ori_dt.do_select_all_w_limit_n_offset()):
      self.copy_source_files_to_target_if_needed(src_rowlist)
  ...
  def lookup_files_for_copy(self, files, relativepath):
    """
    Rules:
    1) file source and target must have the same relativepath
       report if it exists elsewhere
       by verifying its sha1sum equality elsewhere
       in this condition, sha1sum is the important factor
    2) report if copy is needed but name (filename) is already taken
       even if a sha1sum equality is still missing
       in this condition, name is the important factor
    4) if name is available and sha1sum is missing, copy over
       to the same relativepath in destination
    """
    pass

  def process(self):
    for relativepath, dirs, files in os.walk(self.ori_dt):
      self.lookup_files_for_copy(files, relativepath)
