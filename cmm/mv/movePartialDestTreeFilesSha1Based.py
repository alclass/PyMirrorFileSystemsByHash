...
cmm/mv/movePartialDestTreeFilesSha1Based.py
  This script is useful when (duplicated) files, existing both in a dirtree
    and also in a subset (smaller) dirtree,
    need, for unication purposes, to be moved and/or renamed
    based on their relative dir-positions in source.
...
class MoveRename:
  ...
  def move_trg_based_on_src(self, src_dirnode, trg_dirnode):
    ...
    trg_oldpath = os.path.join(self.bak_dt.mountpath, trg_middlepath)
    src_middlepath = src_dirnode.path
    src_middlepath = src_middlepath.lstrip('/')
    trg_newpath = os.path.join(self.bak_dt.mountpath, src_middlepath)
  ...
  def dbupdate_trg_middlepath_with_src(self, src_dirnode, trg_dirnode):
    sql = '''
  ...
  def compare_src_sha1_position_in_trg(self, sha1, _id):
    ...
    tuplevalues = (_id, )
    fetched_rows = self.ori_dt.do_select_with_sql_n_tuplevalues(sql, tuplevalues)
    if not fetched_rows or len(fetched_rows) != 1:
      return False
    src_row = fetched_rows[0]
    sql = 'SELECT * FROM %(tablename)s WHERE sha1=?;'
    tuplevalues = (sha1, )
    fetched_rows = self.bak_dt.do_select_with_sql_n_tuplevalues(sql, tuplevalues)
    if not fetched_rows or len(fetched_rows) != 1:
      return False
    ...
    if src_dirnode.path != trg_dirnode.path:
      if self.move_trg_based_on_src(src_dirnode, trg_dirnode):
        self.dbupdate_trg_middlepath_with_src(src_dirnode, trg_dirnode)

  def traverse_src_nodes(self):
    sql = 'SELECT DISTINCT sha1, COUNT(id), id FROM %(tablename)s GROUP BY sha1;'
    generator_rows = self.ori_dt.do_select_sql_n_tuplevalues_w_limit_n_offset(sql, None)
  ...
  def report(self):
    print('Report:')
    print('=======')
    print('dirtrees:', self.ori_dt.mountpath, self.bak_dt.mountpath)
...
def process():
  ...
  entrydeleter = dbentr_del.DBEntryWithoutCorrespondingOsEntryDeleter(src_mountpath)
  entrydeleter.process()
  entrydeleter = dbentr_del.DBEntryWithoutCorrespondingOsEntryDeleter(trg_mountpath)
  entrydeleter.process()
